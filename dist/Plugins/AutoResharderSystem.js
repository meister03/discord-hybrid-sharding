...
class AutoResharderManager {
    ...
    async checkReCluster() {
        ...
        const reachedCluster = this.clusterDatas.find(v => v?.shardData.some(x => x && x.guildCount >= this.options.MaxGuildsPerShard));
        if (reachedCluster) {
    ...
    initialize() {
        ...
        //             const index = this.clusterDatas.findIndex(v => v && v.v.clusterId === msg.data.clusterId);
