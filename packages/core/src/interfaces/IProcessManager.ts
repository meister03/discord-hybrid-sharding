...
export interface IProcessManager<TProcess extends ISpawnableProcess = ISpawnableProcess> {
  ...
  respawnAll(options?: { processDelay?: number; respawnDelay?: number; timeout?: number }): Promise<void>;
  
  /** Broadcast evaluation to all processes */
  broadcastEval<T>(script: string, options?: { timeout?: number; context?: any }): Promise<T[]>;
